...
Flight::route('POST /authApiKey', function () {
    ...
    if(strlen(Flight::request()->data->apiKey) >=8 && strlen(Flight::request()->data->xApiKey) >= 8){
       ...
       $uu= Flight::request()->data->apiKey;
        $uuq=Flight::request()->data->xApiKey;
        //echo json_encode($info);
echo "Unauthorized ". $uu." ".$uuq
;
//echo json_encode($dta);
    }
...
Flight::route('POST /authApiKeyKairos', function () {
    header("Access-Control-Allow-Origin: *");
    //$conectar = conn();
    //$uri = $_SERVER['REQUEST_URI'];
    $dta = array(
    ...
    if(strlen(Flight::request()->data->apiKey) >=8 && strlen(Flight::request()->data->xApiKey) >= 8){
            ...
            $validate_api_key = new validate_api_key();
            $validation = $validate_api_key->validate_tokenKairos($dta);
...
Flight::route('POST /authApiKeyGateway', function () {
    ...
    if(strlen(Flight::request()->data->ApiKey) >= 8 && strlen(Flight::request()->data->xapiKey) >= 20){
...
Flight::route('POST /authApiKeyGatewayKairos', function () {
    ...
    if(strlen(Flight::request()->data->ApiKey) >= 8 && strlen(Flight::request()->data->xapiKey) >= 8){
            ...
            $validate_api_key = new validate_api_key();
            $validation = $validate_api_key->validate_tokenGatewayKoios($dta);
