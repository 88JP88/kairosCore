...
Flight::route('GET /getInternalUsers/@apiData', function ($apiData) {
...
//echo $apiData;
echo modelGet::getUsersINTERNAL($postData);
...
Flight::route('GET /getInternalClientsClient/@filter/@cid', function ($filter,$cid) {
    header("Access-Control-Allow-Origin: *");
    // Leer los encabezados
    $headers = getallheaders();
    ...
    if (isset($headers['Api-Key']) && isset($headers['x-api-Key'])) {
      ...
      curl_close($curl);
        ...
        if ($response1 == 'true' ) {
            ...
            $conectar=conn();
            if($filter=="unlock"){
                $query= mysqli_query($conectar,"SELECT c.clientId,c.clientName,c.comments,c.isActive,c.status,c.ownerId,c.styleId,c.subId,c.clientType,o.name,o.lastName,o.contact,o.email,s.apiKey FROM clients c JOIN owners o ON c.ownerId=o.ownerId JOIN clientSecrets s ON s.clientId=c.clientId WHERE c.status=1 and c.clientId='$cid'");
           
            }
          
            if($filter=="lock"){
                $query= mysqli_query($conectar,"SELECT c.clientId,c.clientName,c.comments,c.isActive,c.status,c.ownerId,c.styleId,c.subId,c.clientType,o.name,o.lastName,o.contact,o.email,s.apiKey FROM clients c JOIN owners o ON c.ownerId=o.ownerId JOIN clientSecrets s ON s.clientId=c.clientId WHERE c.status=0 and c.clientId='$cid'");
    ...
    if (isset($headers['x-api-Key'])) {
      ...
      curl_close($curl);
        ...
        if ($response1 == 'true' ) {
            ...
            $query= mysqli_query($conectar,"SELECT u.name,u.lastName,u.sessionCounter,c.bgColor,c.textColor,c.imgIcon,c.imgLogo,c.imgGif from generalUsers u JOIN clientStyle c ON u.clientId=c.clientId where u.userId='$userName'");
