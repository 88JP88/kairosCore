...
//DATA EXTRACTION**
        ...
        $query= modelPut::putRoom($postData);  //DATA MODAL
...
//DATA EXTRACTION**
        ...
        $query= modelPost::postRoom($postData);  //DATA MODAL
...
Flight::route('GET /getClientRooms/@apiData/', function ($apiData) {
...
//echo $apiData;
echo modelGet::getRooms($postData);
...
Flight::route('GET /getCalendarTimedes/@apiData/', function ($apiData) {
    header("Access-Control-Allow-Origin: *");
    // Leer los encabezados
    $headers = getallheaders();
    $postData = json_decode($apiData, true);
...
//echo $apiData;
echo modelGet::getCalendarTIMEDES($postData);
...
Flight::route('GET /getElementsAssign/@postData/', function ($apiData) {
    header("Access-Control-Allow-Origin: *");
    // Leer los encabezados
    $headers = getallheaders();
    $postData = json_decode($apiData, true);
...
//echo $apiData;
echo modelGet::getElementsAssign($postData);
    ...
    if (!empty($apk) && !empty($xapk)) {    
      ...
      curl_close($curl);
        ...
        if ($response11 == 'true' ) {
            ...
            $eventId = substr($myuuid, 0, 8);

         
            $conectar=conn();

           
            $query= mysqli_query($conectar,"INSERT INTO fullCalendar (eventId,clientId,userId,startDate,endDate,eventName,roomId,description,urlImage,urlEvent) VALUES ('$eventId','$clientId','$userId','$startDate','$endDate','$eventName','$roomId','$description','$ulrImge','$urlEvent')");
            echo "true|¡Evento creado con éxito!";
...
Flight::route('GET /getFullCalendar/@filter/', function ($filter) {
    header("Access-Control-Allow-Origin: *");
    // Leer los encabezados
    $headers = getallheaders();
    ...
    if (isset($headers['Api-Key']) && isset($headers['x-api-Key'])) {
      ...
      curl_close($curl);
        ...
        $query= mysqli_query($conectar,"SELECT eventId,clientId,userId,startDate,endDate,eventName,classId,roomId,isActive,description,urlImage,urlEvent FROM fullCalendar WHERE clientId='$filter'");
                ...
                $row=$query->fetch_assoc();
                //echo json_encode($students) ;
                echo json_encode(['fullCalendar'=>$values]);
