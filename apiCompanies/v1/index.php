    ...
    if (isset($headers['Api-Key']) && isset($headers['x-api-Key'])) {
      ...
      curl_close($curl);
        ...
        if ($response1 == 'true' ) {
          ...
          if($param=="free"){
            $query= mysqli_query($conectar,"SELECT elementId,elementName,caracts,comments,isActive,status,brand,type,clientId,isApply,imgElements,amount FROM clientElements WHERE clientId='$filter' and isActive=1 and isApply=0 OR clientId='$filter' and roomId='$rid' and isApply=1");
      }
                $values=[];
